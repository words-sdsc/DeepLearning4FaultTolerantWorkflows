...
def create_modelJ1(input_dim, input_length, output_dim):
    print ('Creating model J1 (Pick Train Save)...')
    model = Sequential()
...
def printTestResults(model):
    ...
    print('Model Fingerprint: [80-20]_giga_PickTrainSave Model_J1_10epochsperfile_kube_binar_crosentr_adam_binar_acc.h5')
...
for j in range(len(X_train_files)):
    ...
    if (startup == False):
        model   = create_modelJ1(input_dim, input_length, output_dim)
        startup = True

    print('Fit the Model - Training File # ', j)
    history     = model.fit(X_final,Y_final,batch_size=4*1024*2, epochs=noofiterations, validation_split = 0.10, shuffle=True,verbose= 1)
    print("Saving Model weights...")
    model.save_weights('[80-20]_giga_PickTrainSave Model_J1_10epochsperfile_kube_binar_crosentr_adam_binar_acc.h5')
...
print('Final Testing at end of going through all files')
printTestResults(model)
