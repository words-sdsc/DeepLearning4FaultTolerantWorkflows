...
def load_val_set():
    ...
    for k in range(1): # Pick only 1 file for validation
        ...
        X_valset = np.concatenate(X_valset, X_test)
        Y_valset = np.concatenate(Y_valset, Y_test)
...
#
# checkpoint
filepath        = path + "/comet_model1700by6tsteps2_weights_best.hdf5"
...
history=model.fit_generator(train_generator(),validation_data=(X_valset, Y_valset), steps_per_epoch=totalbatches,epochs=noofiterations,callbacks=callbacks_list, verbose=1,use_multiprocessing=True, workers=1, max_queue_size=2)
########################################################
print("Saving Model weights...")
model.save_weights(path + '/comet_train_[80-20]_giga_long_multithreadedgenerator_PikTranSav_Mdl_J1700_epochs_20_kube_binar_crsentr_adm_binar_acc.h5')
print('Weights Saved')
########################################################
...
print('Testing copy feature for future: Start copy of files...')
