...
def applyParallel(dfGrouped, func):
    global thresholdd
    retLst = Parallel(n_jobs=10)(delayed(func)(group) for name, group in dfGrouped if len(group)>thresholdd-1)
    return retLst
...
def genereate_training_set():
    ...
    for f in setofcleanedfiles:
        ...
        for bat in batch([x for x in range(length)], int(length/breakeachinto)):
            ...
            with open('X_final_giga_long_'+'_file_'+str(minicounter)+'_'+str(counter)+'tstep2.pickle', 'wb') as handle:
            ...
            with open('Y_final_giga_long_'+'_file_'+str(minicounter)+'_'+str(counter)+'tstep2.pickle', 'wb') as handle:
...
def genereate_test_set():
    ...
    for f in setoftakeoutdffiles:
        ...
        for bat in batch([x for x in range(length)], int(length/breakeachinto)):
            ...
            with open('X_test_giga_long_'+'_file_'+str(minicounter)+'_'+str(counter)+'tstep2.pickle', 'wb') as handle:
            ...
            with open('Y_test_giga_long_'+'_file_'+str(minicounter)+'_'+str(counter)+'tstep2.pickle', 'wb') as handle:
